...
for (const t of all) {
  ...
  if (!["Urgent", "High", "Medium", "Low"].includes(t.priority))
    errors.push(`${t.id}: bad priority`);
  if (![1, 2, 3, 5, 8].includes(t.estimate))
    errors.push(`${t.id}: bad estimate`);
  if (!t.acceptance?.length) errors.push(`${t.id}: no acceptance criteria`);
  if (t.description.trim().startsWith("'"))
    errors.push(
      `${t.id}: description starts with an apostrophe (Linear strips it)`,
    );
  for (const d of t.deps)
    if (!byId.has(d)) errors.push(`${t.id}: unknown dep ${d}`);
}
// cycle check
...
for (const [k, e] of Object.entries(EPICS)) {
  ...
  md.push(
    `- **${e.name}** (label \`${e.label}\`, ${n.length} tickets, ${pts} points): ${e.description}`,
  );
}
md.push(``);
...
for (const [k, e] of Object.entries(PRODUCT_TEAMS)) {
  ...
  const pts = n.reduce((s, t) => s + t.estimate, 0);
  md.push(
    `- **${e.epic}** (team ${e.name}, ${n.length} tickets, ${pts} points, ${PHASES[e.phase]}): ${e.description}`,
  );
}
md.push(``);
...
for (const t of all)
  md.push(
    `| ${t.id} | ${t.title} | ${t.team} | ${t.phase} | ${t.priority} | ${t.estimate} | ${t.deps.join(", ") || "—"} |`,
  );
md.push(``);
...
for (const team of teams) {
  ...
  for (const t of rows) {
    if (!groups.has(t.epicName)) groups.set(t.epicName, []);
    groups.get(t.epicName).push(t);
  }
  for (const [epic, list] of groups) {
    ...
    for (const t of list) {
      ...
      md.push(``);
      md.push(
        `**Priority:** ${t.priority} · **Estimate:** ${t.estimate} · **Phase:** ${PHASES[t.phase]} · **Labels:** ${labelsFor(t)}  `,
      );
      md.push(
        `**Depends on:** ${t.deps.length ? t.deps.map((d) => `${d} (${titleOf(d)})`).join("; ") : "none"}`,
      );
      md.push(``);
...
for (const t of all) {
  ...
  stats[t.team].tickets++;
  stats[t.team].points += t.estimate;
