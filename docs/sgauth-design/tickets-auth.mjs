...
export const EPICS = {
  foundation: { name: "E1 Foundation & Neon Migration", label: "epic:foundation", description: "Replace the Supabase foundation with Neon serverless Postgres, Better Auth, Vercel hosting, CI, and environment tooling. SGAuth MUST run on Neon and MUST NOT use Supabase for auth or data." },
  datamodel: { name: "E2 Data Model & Migrations", label: "epic:data-model", description: "Prisma 7 schema on Neon: users, Better Auth tables, positions, admin/Primary Admin invariants, audit log, product registry, security tables, and the migration workflow." },
  authcore: { name: "E3 Authentication Core", label: "epic:auth-core", description: "Email + password authentication with Better Auth: sign-up restricted to northeastern.edu, verification, reset, invites, imported bcrypt hashes, and the login UI." },
  sessions: { name: "E4 Sessions & SSO", label: "epic:sessions", description: "One session across *.northeasternsga.com via a parent-domain cookie, the session endpoint products call, ES256 JWTs + JWKS for Supabase products, logout propagation, redirects, re-authentication, and the non-production topology." },
  admin: { name: "E5 Admin & Primary Admin", label: "epic:admin", description: "Server-enforced admin rules, the single transferable Primary Admin with a guarded transfer flow, break-glass recovery, and bulk user administration." },
  positions: { name: "E6 Positions", label: "epic:positions", description: "Flat, admin-managed positions (stable key + display name) carried in every session; assignment, soft delete, retirement, propagation, and history." },
  ui: { name: "E7 Admin UI & Account UI", label: "epic:ui", description: "Light admin UI (users, positions, products, audit, PA transfer) and a minimal account page for every user (profile, positions, product links, sessions, password, MFA)." },
  sdk: { name: "E8 Product Registry & SDK", label: "epic:sdk", description: "Admin-managed product registry that drives trusted origins and redirects, and the published @sgaoperations/sgauth package products use to read sessions and tokens." },
  security: { name: "E9 Security Hardening", label: "epic:security", description: "Upstash rate limiting, escalating account lockout, CSRF and origin enforcement, headers, TOTP MFA (required for admins), subdomain hygiene, secrets, enumeration resistance, scanning, and a threat model." },
  observability: { name: "E10 Observability & Audit", label: "epic:observability", description: "Append-only audit log with full coverage, structured logs, PostHog analytics and error tracking, health and uptime, alerts, and retention/deletion jobs." },
  docs: { name: "E11 Integration Guides & Docs", label: "epic:docs", description: "Architecture doc (Neon mandate), Neon-product and Supabase-product integration guides, admin runbooks, privacy notice, SDK reference, contributor guide." },
  rollout: { name: "E12 User Migration & Rollout", label: "epic:rollout", description: "Export/import of existing product users (Chambers hashes, Aplio/SenatePath/Attendance emails), launch checklist, rollout comms, cutover and rollback plans." },
  testing: { name: "E13 Testing & QA", label: "epic:testing", description: "Test harness on a Neon test branch, integration tests for every auth flow, authorization matrix, Playwright SSO end-to-end, JWT/JWKS conformance, and load sanity." },
};
...
Reference: Neon Prisma guide (pooled vs direct URLs), Neon free plan limits (10 branches/project, 100 CU-hours/project/month, 0.5 GB storage per project, 6-hour history).`,
  ...
  {
    id: "AUTH-T02", epic: "foundation", phase: 0, priority: "Urgent", estimate: 3, labels: ["neon", "chore"], deps: [],
...
Also triage open branches that build on Supabase (AUTH-7, AUTH-8, auth-9, auth-10, auth-11, AUTH-14): close them with a comment pointing at this design, or cherry-pick any UI-only work that is still useful. Do not merge Supabase code.`,
  ...
  {
    id: "AUTH-T03", epic: "foundation", phase: 0, priority: "Urgent", estimate: 3, labels: ["backend", "better-auth"], deps: ["AUTH-T02", "AUTH-T04"],
    title: "Install Better Auth 1.7 with the Prisma adapter and mount the handler",
...
Plugins (jwt, admin, twoFactor, customSession) are added in their own tickets.`,
  ...
  {
    id: "AUTH-T04", epic: "foundation", phase: 0, priority: "High", estimate: 2, labels: ["backend", "neon", "prisma"], deps: ["AUTH-T01"],
...
- Add \`?sslmode=verify-full\`. Verify whether \`pgbouncer=true\` is needed with Neon's pooler when using a driver adapter and document the result.`,
  ...
  {
    id: "AUTH-T05", epic: "foundation", phase: 0, priority: "High", estimate: 3, labels: ["infra", "vercel", "neon"], deps: ["AUTH-T01", "AUTH-T03"],
    title: "Create the Vercel project, custom domains, environments, and Neon preview branching",
...
- SGAuth's own \`*.vercel.app\` preview deployments cannot set a \`northeasternsga.com\` cookie; they run with \`SGAUTH_ENV=preview\` (host-only cookie, AUTH-T26) so previews are testable in isolation.`,
  ...
  {
    id: "AUTH-T06", epic: "foundation", phase: 0, priority: "High", estimate: 3, labels: ["ci", "chore"], deps: ["AUTH-T03", "AUTH-T93"],
    title: "Update CI: typecheck, lint, format, migrations check, and tests on a Neon test branch",
...
- Required checks + branch protection on \`main\` and \`dev\` (PR required, at least one review, CI green).`,
  ...
  {
    id: "AUTH-T07", epic: "foundation", phase: 0, priority: "Medium", estimate: 1, labels: ["backend", "chore"], deps: ["AUTH-T02"],
  ...
  {
    id: "AUTH-T09", epic: "foundation", phase: 0, priority: "Medium", estimate: 2, labels: ["infra", "neon"], deps: ["AUTH-T05"],
  ...
  {
    id: "AUTH-T10", epic: "datamodel", phase: 0, priority: "High", estimate: 3, labels: ["backend", "prisma"], deps: ["AUTH-T03", "AUTH-T04"],
    title: "Core schema: User plus Better Auth Session, Account, and Verification tables",
...
No NUID, phone, pronouns, or photo. Products keep extra fields keyed by SGAuth user id. Write the initial migration and a Prisma-level test that the Better Auth adapter validates against this schema.`,
  ...
  {
    id: "AUTH-T11", epic: "datamodel", phase: 1, priority: "High", estimate: 3, labels: ["backend", "prisma", "positions"], deps: ["AUTH-T10"],
...
DB-level: partial unique index on \`Position(key) WHERE deletedAt IS NULL\`; CHECK constraint on key format; trigger or service check enforcing a maximum of 50 active positions per user.`,
  ...
  {
    id: "AUTH-T12", epic: "datamodel", phase: 1, priority: "High", estimate: 3, labels: ["backend", "prisma", "admin", "security"], deps: ["AUTH-T10"],
    title: "Primary Admin invariants at the database level and the PrimaryAdminTransfer table",
...
**PrimaryAdminTransfer**: \`id\`, \`fromUserId\`, \`toUserId\`, \`status\` (PENDING_ACCEPTANCE | COOLING | COMPLETED | CANCELLED | EXPIRED), \`createdAt\`, \`acceptanceExpiresAt\` (+7 days), \`acceptedAt\`, \`executesAt\` (acceptedAt + 24 h), \`cancelTokenHash\`, \`completedAt\`, \`cancelledAt\`, \`cancelledById\`, \`reason\`. Partial unique index allowing at most one transfer in PENDING_ACCEPTANCE or COOLING.`,
  ...
  {
    id: "AUTH-T13", epic: "datamodel", phase: 1, priority: "High", estimate: 2, labels: ["backend", "prisma", "audit"], deps: ["AUTH-T10"],
...
\`type\` is validated in TypeScript against the catalog (AUTH-T73), not by a DB CHECK, so adding an event type does not require a migration.`,
  ...
  {
    id: "AUTH-T14", epic: "datamodel", phase: 2, priority: "Medium", estimate: 2, labels: ["backend", "prisma", "registry"], deps: ["AUTH-T10"],
  ...
  {
    id: "AUTH-T15", epic: "datamodel", phase: 1, priority: "Medium", estimate: 2, labels: ["backend", "prisma", "security"], deps: ["AUTH-T10"],
...
- No rate-limit table: rate limiting uses Upstash (AUTH-T63); disable Better Auth's DB-backed limiter storage.`,
  ...
  {
    id: "AUTH-T16", epic: "datamodel", phase: 0, priority: "Medium", estimate: 1, labels: ["backend", "prisma", "docs"], deps: ["AUTH-T04", "AUTH-T05"],
    title: "Migration workflow: deploy in build, never migrate dev against production",
  ...
  {
    id: "AUTH-T17", epic: "authcore", phase: 1, priority: "Urgent", estimate: 3, labels: ["backend", "better-auth"], deps: ["AUTH-T03", "AUTH-T10"],
    title: "Email + password sign-in and self-sign-up restricted to northeastern.edu",
...
No breached-password (HIBP) check, per decision.`,
  ...
  {
    id: "AUTH-T18", epic: "authcore", phase: 1, priority: "High", estimate: 2, labels: ["backend", "email"], deps: ["AUTH-T17", "AUTH-T20", "AUTH-T103"],
  ...
  {
    id: "AUTH-T19", epic: "authcore", phase: 1, priority: "High", estimate: 3, labels: ["backend", "email", "security"], deps: ["AUTH-T17", "AUTH-T20", "AUTH-T103"],
...
- Caps: per-recipient 10 emails per hour and 30 per day; global daily cap (env, default 500) with an alert at 80%. Log every send with template and recipient hash to the log stream.`,
  ...
  {
    id: "AUTH-T21", epic: "authcore", phase: 3, priority: "High", estimate: 2, labels: ["backend", "migration", "security"], deps: ["AUTH-T17"],
...
- Admin UI shows these accounts as 'Password not set'.`,
  ...
  {
    id: "AUTH-T23", epic: "authcore", phase: 2, priority: "Medium", estimate: 2, labels: ["backend", "security"], deps: ["AUTH-T17", "AUTH-T32"],
    title: "Change password (current password + re-auth), revoke other sessions",
  ...
  {
    id: "AUTH-T24", epic: "authcore", phase: 3, priority: "Low", estimate: 2, labels: ["backend", "admin"], deps: ["AUTH-T36", "AUTH-T20"],
    title: "Admin-initiated email change with re-verification and privilege rules",
  ...
  {
    id: "AUTH-T25", epic: "authcore", phase: 1, priority: "High", estimate: 3, labels: ["frontend", "ui"], deps: ["AUTH-T17", "AUTH-T18", "AUTH-T19", "AUTH-T31"],
  ...
  // ───────────────────────── E4 Sessions & SSO ─────────────────────────
  {
    id: "AUTH-T26", epic: "sessions", phase: 1, priority: "Urgent", estimate: 2, labels: ["backend", "sso", "security"], deps: ["AUTH-T03"],
...
Document the threat model: any compromised or dangling \`*.northeasternsga.com\` host can read the cookie **and can set a same-named cookie on the parent domain** (cookie tossing / login CSRF: the victim is silently logged into an attacker-controlled account). Cookie values are signed so they cannot be forged, but \`__Host-\` cannot be used with a Domain attribute, so subdomain hygiene (AUTH-T68) is the control.`,
  ...
  {
    id: "AUTH-T27", epic: "sessions", phase: 1, priority: "High", estimate: 2, labels: ["backend", "sso"], deps: ["AUTH-T10", "AUTH-T26"],
    title: "Session lifetime: 30-day sliding, 90-day absolute cap, no cookie cache",
  ...
  {
    id: "AUTH-T28", epic: "sessions", phase: 1, priority: "Urgent", estimate: 3, labels: ["backend", "sso", "api"], deps: ["AUTH-T27", "AUTH-T11"],
...
- Also expose \`GET /api/sgauth/session/minimal\` returning only user id + positions for hot paths. Publish the JSON schema in the SDK.`,
  ...
  {
    id: "AUTH-T29", epic: "sessions", phase: 2, priority: "High", estimate: 3, labels: ["backend", "sso", "supabase", "jwt"], deps: ["AUTH-T28", "AUTH-T15"],
    title: "ES256 JWTs with JWKS and OIDC discovery for Supabase-backed products",
...
- Token endpoint \`/api/auth/token\` requires a valid session cookie (rate-limited in AUTH-T63).`,
  ...
  {
    id: "AUTH-T30", epic: "sessions", phase: 1, priority: "High", estimate: 2, labels: ["backend", "sso"], deps: ["AUTH-T26", "AUTH-T56"],
...
Supabase-style JWTs already issued remain valid until their 10-minute expiry; document this in the Supabase guide.`,
  ...
  {
    id: "AUTH-T31", epic: "sessions", phase: 1, priority: "High", estimate: 2, labels: ["backend", "security", "sso"], deps: ["AUTH-T56"],
  ...
  {
    id: "AUTH-T32", epic: "sessions", phase: 2, priority: "High", estimate: 3, labels: ["backend", "security"], deps: ["AUTH-T27", "AUTH-T67"],
  ...
  {
    id: "AUTH-T33", epic: "sessions", phase: 2, priority: "Medium", estimate: 2, labels: ["backend"], deps: ["AUTH-T27"],
  ...
  {
    id: "AUTH-T34", epic: "sessions", phase: 1, priority: "High", estimate: 3, labels: ["infra", "sso", "docs"], deps: ["AUTH-T05", "AUTH-T26"],
    title: "Non-production SSO topology: dev deployment, local hostnames, product preview domains",
...
- **SDK dev mode**: the SDK accepts \`SGAUTH_URL\` and \`SGAUTH_COOKIE_NAME\` overrides so products target dev or local SGAuth. Write \`docs/ENVIRONMENTS.md\` with a table of URLs, cookie names, and Neon branches per environment.`,
  ...
  {
    id: "AUTH-T35", epic: "admin", phase: 2, priority: "Urgent", estimate: 3, labels: ["backend", "admin", "security"], deps: ["AUTH-T10", "AUTH-T12"],
...
Return structured denials (\`{ allowed: false, reason: 'SELF_MODIFICATION' | 'PRIMARY_ADMIN_PROTECTED' | 'NOT_ADMIN' | ... }\`). UI hides controls using the same function, but enforcement is server-side only.`,
  ...
  {
    id: "AUTH-T36", epic: "admin", phase: 2, priority: "High", estimate: 5, labels: ["backend", "admin", "api"], deps: ["AUTH-T35", "AUTH-T13", "AUTH-T32", "AUTH-T22"],
  ...
  {
    id: "AUTH-T37", epic: "admin", phase: 2, priority: "High", estimate: 5, labels: ["backend", "admin", "security", "email"], deps: ["AUTH-T12", "AUTH-T35", "AUTH-T32", "AUTH-T20", "AUTH-T67", "AUTH-T103", "AUTH-T38"],
    title: "Primary Admin transfer flow (re-auth, recipient acceptance, 24-hour cancel window)",
...
Exactly one in-flight transfer is allowed. If the recipient loses admin or is deactivated mid-flight, the transfer is cancelled automatically.`,
  ...
  {
    id: "AUTH-T38", epic: "admin", phase: 1, priority: "High", estimate: 2, labels: ["backend", "infra", "ci"], deps: ["AUTH-T05", "AUTH-T07"],
    title: "Scheduled-job runner: GitHub Actions schedules calling secret-protected routes",
...
Each route is idempotent, logs outcomes, and returns quickly (under the function timeout); Actions schedules can be delayed several minutes under load, which is acceptable for these jobs. A missed run is caught by the next.`,
  ...
  {
    id: "AUTH-T39", epic: "admin", phase: 2, priority: "High", estimate: 3, labels: ["backend", "admin", "security", "docs"], deps: ["AUTH-T12", "AUTH-T13"],
  ...
  {
    id: "AUTH-T40", epic: "admin", phase: 2, priority: "High", estimate: 2, labels: ["testing", "admin", "security"], deps: ["AUTH-T12", "AUTH-T36"],
  ...
  // ───────────────────────── E6 Positions ─────────────────────────
  {
    id: "AUTH-T42", epic: "positions", phase: 2, priority: "High", estimate: 3, labels: ["backend", "positions", "api"], deps: ["AUTH-T11", "AUTH-T35", "AUTH-T13"],
    title: "Positions CRUD: create, edit name/category, soft delete with retirement",
  ...
  {
    id: "AUTH-T43", epic: "positions", phase: 2, priority: "High", estimate: 3, labels: ["backend", "positions", "api"], deps: ["AUTH-T42"],
  ...
  {
    id: "AUTH-T44", epic: "positions", phase: 1, priority: "Medium", estimate: 1, labels: ["backend", "positions"], deps: ["AUTH-T11"],
...
Key naming convention (document in the admin UI help and the integration guides): organization roles use bare keys (\`vp-finance\`, \`senator\`); product-specific roles are prefixed with the product slug (\`aplio-admin\`, \`chambers-iems\`) so keys never collide and products can grep their own.`,
  ...
  {
    id: "AUTH-T45", epic: "positions", phase: 2, priority: "Medium", estimate: 2, labels: ["testing", "positions", "sso"], deps: ["AUTH-T43", "AUTH-T28", "AUTH-T29"],
  ...
  {
    id: "AUTH-T46", epic: "positions", phase: 3, priority: "Low", estimate: 2, labels: ["backend", "positions", "audit"], deps: ["AUTH-T42", "AUTH-T73"],
    ...
    acceptance: ["Both queries paginate and return within 200 ms for 10k events on the test branch."],
  },

  // ───────────────────────── E7 Admin UI & Account UI ─────────────────────────
  {
    id: "AUTH-T47", epic: "ui", phase: 2, priority: "High", estimate: 3, labels: ["frontend", "ui"], deps: ["AUTH-T25", "AUTH-T28"],
  ...
  {
    id: "AUTH-T48", epic: "ui", phase: 2, priority: "High", estimate: 5, labels: ["frontend", "ui", "admin"], deps: ["AUTH-T47", "AUTH-T36", "AUTH-T43"],
  ...
  {
    id: "AUTH-T49", epic: "ui", phase: 2, priority: "High", estimate: 3, labels: ["frontend", "ui", "positions"], deps: ["AUTH-T47", "AUTH-T42"],
  ...
  {
    id: "AUTH-T50", epic: "ui", phase: 2, priority: "Medium", estimate: 2, labels: ["frontend", "ui", "registry"], deps: ["AUTH-T47", "AUTH-T56"],
    ...
    acceptance: ["Adding a product makes its origin trusted and its URL appear on account pages without a deploy (verified on dev)."],
  },
  {
    id: "AUTH-T51", epic: "ui", phase: 3, priority: "Medium", estimate: 3, labels: ["frontend", "ui", "audit"], deps: ["AUTH-T47", "AUTH-T73"],
    ...
    acceptance: ["Filtering by a user shows both events they performed and events targeting them.", "Export matches the on-screen filter."],
  },
  {
    id: "AUTH-T52", epic: "ui", phase: 2, priority: "High", estimate: 3, labels: ["frontend", "ui", "admin"], deps: ["AUTH-T47", "AUTH-T37"],
    title: "Admin: Primary Admin transfer wizard, status, acceptance, and cancel pages",
  ...
  {
    id: "AUTH-T53", epic: "ui", phase: 2, priority: "High", estimate: 3, labels: ["frontend", "ui"], deps: ["AUTH-T47", "AUTH-T33", "AUTH-T56"],
    title: "Account page: profile, positions, product links, sessions, sign out everywhere",
  ...
  {
    id: "AUTH-T54", epic: "ui", phase: 3, priority: "High", estimate: 3, labels: ["frontend", "ui", "security"], deps: ["AUTH-T23", "AUTH-T67"],
    title: "Account security page: change password, MFA enrollment, backup codes",
  ...
  {
    id: "AUTH-T55", epic: "ui", phase: 3, priority: "Medium", estimate: 2, labels: ["frontend", "ui", "a11y"], deps: ["AUTH-T48", "AUTH-T49", "AUTH-T53", "AUTH-T54"],
    ...
    acceptance: ["axe: zero serious/critical issues on all pages; documented checklist completed."],
  },

  // ───────────────────────── E8 Product Registry & SDK ─────────────────────────
  {
    id: "AUTH-T56", epic: "sdk", phase: 1, priority: "High", estimate: 2, labels: ["backend", "registry", "security"], deps: ["AUTH-T14"],
    title: "Product registry service: trusted origins and redirect allowlist at runtime",
  ...
  {
    id: "AUTH-T57", epic: "sdk", phase: 2, priority: "High", estimate: 3, labels: ["sdk", "infra"], deps: ["AUTH-T28"],
    title: "Scaffold the @sgaoperations/sgauth package and publish pipeline (public npm)",
  ...
  {
    id: "AUTH-T58", epic: "sdk", phase: 2, priority: "High", estimate: 3, labels: ["sdk", "sso"], deps: ["AUTH-T57"],
  ...
  {
    id: "AUTH-T59", epic: "sdk", phase: 2, priority: "High", estimate: 3, labels: ["sdk", "sso"], deps: ["AUTH-T58"],
    title: "SDK: Next.js helpers (proxy/middleware, requireSession, position guards, URLs)",
  ...
  {
    id: "AUTH-T60", epic: "sdk", phase: 3, priority: "Medium", estimate: 2, labels: ["sdk", "supabase", "jwt"], deps: ["AUTH-T58", "AUTH-T29"],
    ...
    acceptance: ["Token refresh happens before expiry in a long-running test; a revoked session stops yielding tokens at the next refresh."],
  },
  {
    id: "AUTH-T61", epic: "sdk", phase: 2, priority: "Medium", estimate: 2, labels: ["sdk", "docs"], deps: ["AUTH-T59"],
    ...
    acceptance: ["A new product can integrate using only the README (validated by the VaultZ integration)."],
  },
  {
    id: "AUTH-T62", epic: "sdk", phase: 3, priority: "Medium", estimate: 2, labels: ["backend", "security", "sso"], deps: ["AUTH-T56", "AUTH-T28"],
    ...
    acceptance: ["Preflight from an unregistered origin gets no CORS headers; from a registered one it succeeds with credentials."],
  },

  // ───────────────────────── E9 Security Hardening ─────────────────────────
  {
    id: "AUTH-T63", epic: "security", phase: 1, priority: "High", estimate: 3, labels: ["backend", "security", "infra"], deps: ["AUTH-T03", "AUTH-T07"],
  ...
  {
    id: "AUTH-T64", epic: "security", phase: 1, priority: "High", estimate: 5, labels: ["backend", "security", "email"], deps: ["AUTH-T15", "AUTH-T17", "AUTH-T20", "AUTH-T103"],
    title: "Escalating account lockout with emailed unlock and known-device exemption",
  ...
  {
    id: "AUTH-T65", epic: "security", phase: 1, priority: "High", estimate: 2, labels: ["backend", "security", "sso"], deps: ["AUTH-T56", "AUTH-T26"],
  ...
  {
    id: "AUTH-T66", epic: "security", phase: 2, priority: "Medium", estimate: 2, labels: ["backend", "security"], deps: ["AUTH-T05"],
    ...
    acceptance: ["securityheaders.com grade A on production; CSP violations reported to PostHog or a report-only endpoint first for one week."],
  },
  {
    id: "AUTH-T67", epic: "security", phase: 2, priority: "High", estimate: 5, labels: ["backend", "security", "better-auth"], deps: ["AUTH-T15", "AUTH-T17"],
    title: "TOTP multi-factor authentication (optional for users, required for admins and the Primary Admin)",
  ...
  {
    id: "AUTH-T68", epic: "security", phase: 2, priority: "Medium", estimate: 2, labels: ["infra", "security", "docs"], deps: ["AUTH-T26"],
    title: "Subdomain hygiene: DNS inventory, dangling-record removal, and policy",
    ...
    acceptance: ["Inventory spreadsheet/link attached; zero dangling records; policy merged and linked from the architecture doc."],
  },
  {
    id: "AUTH-T69", epic: "security", phase: 2, priority: "Medium", estimate: 1, labels: ["infra", "security", "docs"], deps: ["AUTH-T05", "AUTH-T29"],
    ...
    acceptance: ["`docs/runbooks/rotate-secrets.md` exists and a dry run of the JWKS rotation on dev against a throwaway Supabase project produces zero token rejections during the switch."],
  },
  {
    id: "AUTH-T70", epic: "security", phase: 1, priority: "Medium", estimate: 2, labels: ["backend", "security", "testing"], deps: ["AUTH-T17", "AUTH-T19"],
    ...
    acceptance: ["Tests assert identical response bodies and status codes across known/unknown emails for each endpoint; a non-gating benchmark script reports timing deltas (a hard 50 ms CI assertion was rejected as flaky)."],
  },
  {
    id: "AUTH-T71", epic: "security", phase: 3, priority: "Low", estimate: 1, labels: ["ci", "security"], deps: ["AUTH-T06"],
    ...
    acceptance: ["All three run on the repo; a seeded vulnerable dependency fails CI in a test PR."],
  },
  {
    id: "AUTH-T72", epic: "security", phase: 2, priority: "Medium", estimate: 3, labels: ["docs", "security"], deps: ["AUTH-T26", "AUTH-T29", "AUTH-T35"],
    ...
    acceptance: ["Document reviewed by at least two team members; every residual risk has an owner or an accepted-risk note."],
  },

  // ───────────────────────── E10 Observability & Audit ─────────────────────────
  {
    id: "AUTH-T73", epic: "observability", phase: 1, priority: "High", estimate: 3, labels: ["backend", "audit"], deps: ["AUTH-T13"],
  ...
  {
    id: "AUTH-T74", epic: "observability", phase: 1, priority: "High", estimate: 2, labels: ["backend", "observability"], deps: ["AUTH-T03"],
    ...
    acceptance: ["Sample production log line validated against a schema; a test asserts secrets are redacted."],
  },
  {
    id: "AUTH-T75", epic: "observability", phase: 2, priority: "Medium", estimate: 2, labels: ["observability", "posthog"], deps: ["AUTH-T74"],
    ...
    acceptance: ["Events appear in PostHog from dev with no PII properties (verified by inspecting event payloads); an induced server error shows in error tracking."],
  },
  {
    id: "AUTH-T76", epic: "observability", phase: 1, priority: "Medium", estimate: 1, labels: ["backend", "infra"], deps: ["AUTH-T04", "AUTH-T29"],
    ...
    acceptance: ["Monitor is live and alerted correctly during a deliberate 10-minute dev outage test."],
  },
  {
    id: "AUTH-T77", epic: "observability", phase: 3, priority: "Low", estimate: 2, labels: ["backend", "observability", "email"], deps: ["AUTH-T73", "AUTH-T20", "AUTH-T38"],
    ...
    acceptance: ["Simulated spike triggers exactly one alert email; the same condition an hour later triggers again."],
  },
  {
    id: "AUTH-T78", epic: "observability", phase: 3, priority: "Medium", estimate: 3, labels: ["backend", "privacy", "infra"], deps: ["AUTH-T13", "AUTH-T36", "AUTH-T20", "AUTH-T38"],
    title: "Retention jobs: tombstone deactivated (30 d) and inactive (12 mo) users, purge sessions and PII",
  ...
  {
    id: "AUTH-T79", epic: "docs", phase: 1, priority: "High", estimate: 3, labels: ["docs"], deps: ["AUTH-T26", "AUTH-T28", "AUTH-T29"],
    ...
    acceptance: ["Doc merged at `docs/ARCHITECTURE.md` with Mermaid diagrams that render on GitHub; reviewed by the team lead."],
  },
  {
    id: "AUTH-T80", epic: "docs", phase: 2, priority: "High", estimate: 3, labels: ["docs", "sdk"], deps: ["AUTH-T61", "AUTH-T34"],
    ...
    acceptance: ["VaultZ integration completed by following the guide with no undocumented steps (feedback folded back in)."],
  },
  {
    id: "AUTH-T81", epic: "docs", phase: 3, priority: "High", estimate: 5, labels: ["docs", "supabase", "jwt"], deps: ["AUTH-T29", "AUTH-T60"],
    title: "Integration guide for Supabase-backed products (third-party auth) plus the move-to-Neon alternative",
...
8. **Move to Neon instead**: checklist for migrating a Supabase product's Postgres to Neon and using the standard Neon guide, recommended for any product that needs auth and does not depend on Supabase-only features (Storage, Realtime).`,
  ...
  {
    id: "AUTH-T82", epic: "docs", phase: 2, priority: "Medium", estimate: 3, labels: ["docs", "admin"], deps: ["AUTH-T37", "AUTH-T39", "AUTH-T64", "AUTH-T41"],
    ...
    acceptance: ["Each runbook has been executed once on dev by someone other than its author and corrected accordingly."],
  },
  {
    id: "AUTH-T83", epic: "docs", phase: 3, priority: "Medium", estimate: 2, labels: ["docs", "privacy"], deps: ["AUTH-T78"],
    ...
    acceptance: ["Page live and linked from the login footer; reviewed by the Primary Admin."],
  },
  {
    id: "AUTH-T84", epic: "docs", phase: 3, priority: "Low", estimate: 1, labels: ["docs", "sdk"], deps: ["AUTH-T61"],
    ...
    acceptance: ["Reference site live; a PR without a changelog entry fails CI."],
  },
  {
    id: "AUTH-T85", epic: "docs", phase: 1, priority: "Low", estimate: 1, labels: ["docs", "chore"], deps: ["AUTH-T02", "AUTH-T06"],
    ...
    acceptance: ["CLAUDE.md merged; agent definitions no longer mention Supabase."],
  },

  // ───────────────────────── E12 User Migration & Rollout ─────────────────────────
  {
    id: "AUTH-T86", epic: "rollout", phase: 3, priority: "High", estimate: 2, labels: ["migration", "chambers"], deps: [],
    title: "Receive the Chambers auth.users export and define the import file format",
    ...
    acceptance: ["Schema and validator merged; the received export validates with counts matching what Eli reports from Supabase."],
  },
  {
    id: "AUTH-T87", epic: "rollout", phase: 3, priority: "High", estimate: 3, labels: ["migration", "backend", "chambers"], deps: ["AUTH-T86", "AUTH-T21", "AUTH-T44", "AUTH-T13"],
    title: "Import script: Chambers users with bcrypt hashes and position mapping",
  ...
  {
    id: "AUTH-T90", epic: "rollout", phase: 3, priority: "Medium", estimate: 2, labels: ["docs", "rollout"], deps: ["AUTH-T80", "AUTH-T87"],
    ...
    acceptance: ["Plan reviewed with each product owner; dates recorded; announcement emails drafted."],
  },
  {
    id: "AUTH-T91", epic: "rollout", phase: 2, priority: "High", estimate: 2, labels: ["rollout", "infra", "security"], deps: ["AUTH-T72", "AUTH-T66", "AUTH-T76", "AUTH-T39", "AUTH-T64"],
    ...
    acceptance: ["Checklist completed and signed off in the ticket by the Primary Admin; bootstrap script left disabled afterwards."],
  },
  {
    id: "AUTH-T92", epic: "rollout", phase: 4, priority: "Low", estimate: 1, labels: ["rollout", "docs"], deps: ["AUTH-T90"],
    ...
    acceptance: ["Review notes recorded for every product; legacy-cleanup subtasks closed."],
  },

  // ───────────────────────── E13 Testing & QA ─────────────────────────
  {
    id: "AUTH-T93", epic: "testing", phase: 0, priority: "High", estimate: 3, labels: ["testing", "infra"], deps: ["AUTH-T01", "AUTH-T04"],
    ...
    acceptance: ["`npm test` runs locally against a personal branch and in CI against the test branch in under 5 minutes.", "Factories and fakes documented in CONTRIBUTING.md."],
  },
  {
    id: "AUTH-T94", epic: "testing", phase: 1, priority: "High", estimate: 5, labels: ["testing", "auth-core"], deps: ["AUTH-T93", "AUTH-T17", "AUTH-T18", "AUTH-T19", "AUTH-T27", "AUTH-T64"],
    ...
    acceptance: ["All flows green in CI; each test asserts the expected audit events."],
  },
  {
    id: "AUTH-T95", epic: "testing", phase: 2, priority: "High", estimate: 3, labels: ["testing", "admin"], deps: ["AUTH-T93", "AUTH-T36", "AUTH-T37", "AUTH-T42", "AUTH-T43"],
    ...
    acceptance: ["Matrix and state-machine tests pass; coverage of `authz.ts` and the transfer service ≥ 90%."],
  },
  {
    id: "AUTH-T96", epic: "testing", phase: 2, priority: "High", estimate: 5, labels: ["testing", "sso", "e2e"], deps: ["AUTH-T34", "AUTH-T59", "AUTH-T30"],
    ...
    acceptance: ["Playwright job green in CI with traces on failure; run time under 4 minutes."],
  },
  {
    id: "AUTH-T97", epic: "testing", phase: 2, priority: "Medium", estimate: 2, labels: ["testing", "jwt", "supabase"], deps: ["AUTH-T29"],
    ...
    acceptance: ["Conformance suite green; a checklist in the Supabase guide references these tests."],
  },
  {
    id: "AUTH-T98", epic: "testing", phase: 3, priority: "Low", estimate: 2, labels: ["testing", "performance"], deps: ["AUTH-T28", "AUTH-T05"],
    ...
    acceptance: ["p95 < 250 ms and zero connection errors at 100 VUs; results in `docs/PERFORMANCE.md`."],
  },

  // ───────────────────────── Backlog / spikes ─────────────────────────
  {
    id: "AUTH-T99", epic: "authcore", phase: 5, priority: "Low", estimate: 3, labels: ["spike", "sso"], deps: ["AUTH-T91"],
    ...
    acceptance: ["Written findings with a go/no-go recommendation and, if go, a follow-up ticket set."],
  },
  {
    id: "AUTH-T100", epic: "authcore", phase: 5, priority: "Low", estimate: 2, labels: ["backlog", "email"], deps: ["AUTH-T91"],
    ...
    acceptance: ["Decision recorded after reviewing 60 days of email volume metrics."],
  },

  // ───────────────────────── Red-team additions ─────────────────────────
  {
    id: "AUTH-T101", epic: "foundation", phase: 0, priority: "High", estimate: 2, labels: ["infra", "neon", "observability"], deps: ["AUTH-T01"],
...
Mitigations: (1) a scheduled job (AUTH-T38, daily) reads consumption via the Neon API and emails all admins at 50%, 70%, and 85% of the monthly CU-hour quota, with a projection of the exhaustion date; (2) keep scale-to-zero at 5 minutes and rely on the SDK cache so idle periods suspend the compute; (3) \`docs/runbooks/neon-upgrade.md\`: one-click upgrade to Launch (pay-as-you-go, $0.106/CU-hour), what changes (7-day restore window, scale-to-zero configurable), and who is authorized to approve the spend; (4) record the 6-hour restore window as a known limitation in the architecture doc; (5) never add keep-warm pings (they burn the quota).`,
  ...
  {
    id: "AUTH-T103", epic: "authcore", phase: 1, priority: "Urgent", estimate: 2, labels: ["backend", "email", "security"], deps: ["AUTH-T20"],
    title: "Scanner-safe email links: land on a page, consume the token on POST",
...
- Tombstoning (AUTH-T78) and admin deactivation remove passkeys.`,
  ...
  {
    id: "AUTH-T104", epic: "sessions", phase: 1, priority: "Urgent", estimate: 2, labels: ["spike", "sso", "better-auth"], deps: ["AUTH-T05", "AUTH-T26"],
    title: "Spike: validate cross-subdomain cookies, prefixes, and local hostnames on the real domain before building on them",
