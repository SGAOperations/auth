...
| Email | Resend on a dedicated SGAuth sending domain and API key, provider-agnostic mailer interface, per-recipient and global caps | Keeps SGAuth off Aplio's and Chambers' quota; volume is low with password login |
