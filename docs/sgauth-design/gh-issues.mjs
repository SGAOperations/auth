...
const REPOS = { AUTH: "auth", VAULTZ: "vaultz", CHAMBERS: "chambers", APLIO: "aplio", SENATEPATH: "senate-path" }; // ATTENDANCE (SenatePortal) skipped
...
const saveMap = () => writeFileSync(MAP_FILE, JSON.stringify(map, null, 2) + "\n");
...
function depRef(id, fromTeam) {
  ...
  const link = sameRepo ? `#${m.number}` : `${ORG}/${REPOS[t.team]}#${m.number}`;
  return `${link} — ${id} ${t.title}`;
}

function body(t) {
  const deps = t.deps.length ? t.deps.map((d) => `- ${depRef(d, t.team)}`).join("\n") : "- none";
  return [
...
async function createLabels(team) {
  ...
  const names = new Set();
  for (const t of all.filter((x) => x.team === team)) labelsFor(t).forEach((l) => names.add(l));
  for (const name of names) {
    ...
    try {
      ...
      console.error(`label FAILED: ${repo} ${name}: ${String(e.stderr || e.message).trim()}`);
      return false;
    }
  }
  return true;
...
async function create(team, { withLabels }) {
  ...
  let n = 0;
  for (const t of rows) {
    if (map[t.id]) { console.log(`skip (exists): ${t.id} -> #${map[t.id].number}`); continue; }
    const payload = { title: t.title, body: body(t) };
    if (withLabels) payload.labels = labelsFor(t);
    const file = join(TMP, `${t.id}.json`);
    writeFileSync(file, JSON.stringify(payload));
    try {
      const res = JSON.parse(gh(["api", `repos/${repo}/issues`, "--method", "POST", "--input", file]));
...
async function link() {
  ...
  for (const t of all) {
    ...
    try {
      ...
      console.error(`FAILED link ${t.id}: ${String(e.stderr || e.message).trim()}`);
      process.exit(1);
    }
    await sleep(1500);
  }
